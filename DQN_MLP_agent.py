...
def plot_illegal_action_rate(N_evaluates, illegal_action_rates, path, error_rates_curriculum):
    ...
    plt.legend()
    plt.savefig(f'Figure_results/Results_illegal_actions/dqn/benchmark_MWPM_curriculum_{path}.pdf')
    plt.show()
...
def plot_benchmark_MWPM(success_rates_all, success_rates_all_MWPM,error_rates_eval, board_size,path_plot,agent_value_N, agent_value_error_rate,evaluate_fixed):
    ...
    if evaluate_fixed:
        ...
        plt.scatter(N_evaluates, success_rates_all[0,:]*100, label=f"d={board_size} DQN agent, N={agent_value_N}", marker="^", s=30)
        ...
        plt.scatter(error_rates_eval, success_rates_all[0,:]*100, label=f"d={board_size} DQN agent, p_error={agent_value_error_rate}", marker="^", s=30)
        plt.plot(error_rates_eval, success_rates_all[0,:]*100, linestyle='-.', linewidth=0.5)
    plt.title(r'Toric Code - DQN vs MWPM')
    plt.xlabel(r'N')
...
class DQN_agent:
    ...
    def initialise_model(self):
        ...
        #INITIALISE MODEL FOR INITIALISATION
        self.model = DQN("MlpPolicy", self.env, learning_rate=self.initialisation_settings['lr'], verbose=0)
    ...
    def load_model(self, load_model_path):
        ...
        #self.model=DQN.load(f"trained_models/ppo_{load_model_path}")
        self.model=DQN.load(f"trained_models/ppo_{load_model_path}")
        print("loading done")
    ...
    def evaluate_model(self, evaluation_settings, render, number_evaluations, max_moves, check_fails):
        ...
        for k in range(number_evaluations):
            ...
            initial_flips = AgentDQN.env.initial_qubits_flips
    ...
    def decode_MWPM_method(self,obs0_k, initial_flips, evaluation_settings):
        ...
        qubit_pos = AgentDQN.env.state.qubit_pos
    ...
    def evaluate_fixed_errors(self, evaluation_settings, N_evaluates, render, number_evaluations, max_moves, check_fails, save_files):
        ...
        if save_files:
            if fixed:
                np.savetxt(f"Files_results/success_rates_agent/dqn/success_rates_ppo_{evaluation_path}_{loaded_model_settings['N']}.csv", success_rates)
                np.savetxt(f"Files_results/success_rates_MWPM/dqn/success_rates_ppo_{evaluation_path}_{loaded_model_settings['N']}.csv", success_rates_MWPM)
                np.savetxt(f"Files_results/observations/dqn/observations_ppo_{evaluation_path}_{loaded_model_settings['N']}.csv", observations)
                np.savetxt(f"Files_results/results_agent_MWPM/dqn/results_ppo_{evaluation_path}_{loaded_model_settings['N']}.csv", results)
                np.savetxt(f"Files_results/actions_agent_MWPM/dqn/actions_agent_ppo_{evaluation_path}_{loaded_model_settings['N']}.csv", actions[:,:,0])
                np.savetxt(f"Files_results/actions_agent_MWPM/dqn/actions_MWPM_ppo_{evaluation_path}_{loaded_model_settings['N']}.csv", actions[:,:,1])
            else:
                np.savetxt(f"Files_results/success_rates_agent/dqn/success_rates_ppo_{evaluation_path}_{loaded_model_settings['error_rate']}.csv", success_rates)
                np.savetxt(f"Files_results/success_rates_MWPM/dqn/success_rates_ppo_{evaluation_path}_{loaded_model_settings['error_rate']}.csv", success_rates_MWPM)
                np.savetxt(f"Files_results/observations/dqn/observations_ppo_{evaluation_path}_{loaded_model_settings['error_rate']}.csv", observations)
                np.savetxt(f"Files_results/results_agent_MWPM/dqn/results_ppo_{evaluation_path}_{loaded_model_settings['error_rate']}.csv", results)
                np.savetxt(f"Files_results/actions_agent_MPWM/dqn/actions_agent_ppo_{evaluation_path}_{loaded_model_settings['error_rate']}.csv", actions[:,:,0])
                np.savetxt(f"Files_results/actions_agent_MPWM/dqn/actions_MWPM_ppo_{evaluation_path}_{loaded_model_settings['error_rate']}.csv", actions[:,:,1])
...
log = False #if set to True the learning curve during training is registered and saved.
lambda_value=1
...
for curriculum_val in curriculums:
    ...
    if train:
        ...
        #print(f"{loaded_model_settings['s_reward']=}")
    ...
    if curriculum:
        if fixed:
            ...
            #initialisation_settings['s_reward']=curriculum_val
        else:
            print(f"error_rate_curriculum={curriculum_val}")
            initialisation_settings['error_rate']=curriculum_val
            #initialisation_settings['success_reward']=initialisation_settings['N']
        ...
        if fixed:
            ...
            #loaded_model_settings['s_reward']=curriculum_val
        else:
    ...
    if evaluate:
        ...
        if evaluate_fixed:
            success_rates, success_rates_MWPM,observations, results, actions = AgentDQN.evaluate_fixed_errors(evaluation_settings, N_evaluates, render, number_evaluations, max_moves, check_fails, save_files)
        else:
            success_rates, success_rates_MWPM,observations, results, actions = AgentDQN.evaluate_error_rates(evaluation_settings, error_rates, render, number_evaluations, max_moves, check_fails, save_files, fixed)
...
if fixed:
    path = f"Figure_results/Results_benchmarks/dqn/benchmark_MWPM_{evaluation_path}_{loaded_model_settings['N']}.pdf"
else:
    path = f"Figure_results/Results_benchmarks/dqn/benchmark_MWPM_{evaluation_path}_{loaded_model_settings['error_rate']}.pdf"
...
if fixed:
    path_plot = f"Figure_results/Results_benchmarks/dqn/DQN_vs_MWPM_{evaluation_path}_{loaded_model_settings['N']}.pdf"
else:
    path_plot = f"Figure_results/Results_benchmarks/dqn/DQN_vs_MWPM_{evaluation_path}_{loaded_model_settings['error_rate']}.pdf"
