...
from toric_game_env_cnn import ToricGameEnv, ToricGameEnvLocalErrs, ToricGameEnvFixedErrs, ToricGameEnvLocalFixedErrs
...
def plot_illegal_action_rate(N_evaluates, illegal_action_rates, path, error_rates_curriculum):
    ...
    plt.legend()
    plt.savefig(f'Figure_results/Results_illegal_actions/cnn/benchmark_MWPM_curriculum_{path}.pdf')
    plt.show()
...
class PPO_agent:
    ...
    def initialise_model(self):
        ...
        if self.initialisation_settings['random_error_distr']:
            ...
            if self.initialisation_settings['fixed']:
                self.env = ToricGameEnvLocalFixedErrs(self.initialisation_settings)
            else: 
        ...
        if self.initialisation_settings['mask_actions']:
            self.model = MaskablePPO(MaskableActorCriticCnnPolicy, self.env, learning_rate=self.initialisation_settings['lr'], verbose=0,policy_kwargs=dict(normalize_images=False))
        else:
            self.model = PPO(CnnPolicy, self.env, batch_size=64, ent_coef=0.0 , vf_coef=0.5, learning_rate=self.initialisation_settings['lr'], verbose=0,policy_kwargs=dict(normalize_images=False))
    ...
    def change_environment_settings(self, settings):
        ...
        if settings['random_error_distr']:
            ...
            if settings['fixed']:
                self.env = ToricGameEnvLocalFixedErrs(settings)
            else: 
    ...
    def train_model(self, save_model_path):
        ...
        self.model.save(f"trained_models/cnn_models/ppo_{save_model_path}")
        print("training done")
    ...
    def load_model(self, load_model_path):
        ...
        if self.initialisation_settings['mask_actions']:
            self.model=MaskablePPO.load(f"trained_models/cnn_models/ppo_{load_model_path}")
            #self.model=MaskablePPO.load(f"trained_models/ppo_board_size=5error_model=0error_rate=0.01l_reward=5s_reward=10c_reward=-1i_reward=-1000lr=<function linear_schedule.<locals>.func at 0x136839da0>total_timesteps=200000random_error_distr=Truemask_actions=Truelambda=1fixed=TrueN=4")
        else:
            self.model=PPO.load(f"trained_models/cnn_models/ppo_{load_model_path}")
        print("loading done")
    ...
    def plot_results(self,log_folder, save_model_path, title="Learning Curve"):
        ...
        plt.savefig(f'Figure_results/Results_reward_logs/cnn/learning_curve_{save_model_path}.pdf')
        #plt.show()
    ...
    def evaluate_fixed_errors(self, evaluation_settings, N_evaluates, render, number_evaluations, max_moves, check_fails, save_files):
        ...
        if save_files:
            if fixed:
                np.savetxt(f"Files_results/success_rates_agent/cnn/success_rates_ppo_{evaluation_path}_{loaded_model_settings['N']}.csv", success_rates)
                np.savetxt(f"Files_results/success_rates_MWPM/cnn/success_rates_ppo_{evaluation_path}_{loaded_model_settings['N']}.csv", success_rates_MWPM)
                np.savetxt(f"Files_results/observations/cnn/observations_ppo_{evaluation_path}_{loaded_model_settings['N']}.csv", observations)
                np.savetxt(f"Files_results/results_agent_MWPM/cnn/results_ppo_{evaluation_path}_{loaded_model_settings['N']}.csv", results)
                np.savetxt(f"Files_results/actions_agent_MWPM/cnn/actions_agent_ppo_{evaluation_path}_{loaded_model_settings['N']}.csv", actions[:,:,0])
                np.savetxt(f"Files_results/actions_agent_MWPM/cnn/actions_MWPM_ppo_{evaluation_path}_{loaded_model_settings['N']}.csv", actions[:,:,1])
            else:
                np.savetxt(f"Files_results/success_rates_agent/cnn/success_rates_ppo_{evaluation_path}_{loaded_model_settings['error_rate']}.csv", success_rates)
                np.savetxt(f"Files_results/success_rates_MWPM/cnn/success_rates_ppo_{evaluation_path}_{loaded_model_settings['error_rate']}.csv", success_rates_MWPM)
                np.savetxt(f"Files_results/observations/cnn/observations_ppo_{evaluation_path}_{loaded_model_settings['error_rate']}.csv", observations)
                np.savetxt(f"Files_results/results_agent_MWPM/cnn/results_ppo_{evaluation_path}_{loaded_model_settings['error_rate']}.csv", results)
                np.savetxt(f"Files_results/actions_agent_MPWM/cnn/actions_agent_ppo_{evaluation_path}_{loaded_model_settings['error_rate']}.csv", actions[:,:,0])
                np.savetxt(f"Files_results/actions_agent_MPWM/cnn/actions_MWPM_ppo_{evaluation_path}_{loaded_model_settings['error_rate']}.csv", actions[:,:,1])
...
illegal_action_reward=-1000 #the reward the agent gets when mask_actions is set to False and therefore the agent gets penalized by choosing an illegal action.
...
if fixed:
    path = f"Figure_results/Results_benchmarks/cnn/benchmark_MWPM_{evaluation_path}_{loaded_model_settings['N']}.pdf"
else:
    path = f"Figure_results/Results_benchmarks/cnn/benchmark_MWPM_{evaluation_path}_{loaded_model_settings['error_rate']}.pdf"
...
if fixed:
    path_plot = f"Figure_results/Results_benchmarks/cnn/PPO_vs_MWPM_{evaluation_path}_{loaded_model_settings['N']}.pdf"
else:
    path_plot = f"Figure_results/Results_benchmarks/cnn/PPO_vs_MWPM_{evaluation_path}_{loaded_model_settings['error_rate']}.pdf"
